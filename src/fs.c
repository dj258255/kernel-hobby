...
static int      log_n;
static int      log_overflow;        // 트랜잭션이 로그(LOGN)보다 크면 1 → 커밋 거부
...
static void log_begin(void) { log_n = 0; log_overflow = 0; }

// 목적지 dst로 갈 데이터를 로그 데이터 블록에 먼저 적어둔다(아직 제자리엔 안 씀).
static void log_write(uint32 dst, const uint8 *data) {
    if (log_n >= LOGN) { log_overflow = 1; return; }  // 로그보다 큰 트랜잭션 → 표시만, 커밋은 거부됨
...
static void log_commit(void) {
    if (log_n == 0) return;
    if (log_overflow) {                // 로그를 넘긴 트랜잭션은 부분 커밋하면 디스크가 깨짐 → 통째로 버린다
        uart_puts("[fs] transaction too large -- aborted (no partial commit)\n");
        log_n = 0;
        log_overflow = 0;
        return;
    }
