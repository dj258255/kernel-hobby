...
| **백그라운드 / 데몬** | 셸 `cmd &`, 작업 제어(세션·프로세스 그룹), 데몬화(double-fork + `setsid`로 제어 터미널 분리) | 중 |
| **커널 스레드(주소공간 공유)** | `clone()`식 — 한 프로세스 안 여러 흐름을 **커널이** 스케줄해 멀티코어 활용(우리 uthread는 유저공간 협조적이라 1코어) | 중상 |
| **프로세스 생애 정제** | 고아 init 재양육, 좀비 회수, 우선순위(`nice`), `waitpid`/`WNOHANG`, `kill` | 중 |
| **스왑 / 페이지 교체** | 메모리 부족 시 페이지를 디스크로 evict(LRU/Clock). virtio-blk·저널링이 토대 | 중상 |
| **슬랩 / 버디 할당기** | 단순 freelist → 객체 캐싱 할당기로 단편화↓ (Bonwick 슬랩) | 중 |
| **VM 정제: 거대 페이지** | Sv39 2MB/1GB 메가페이지로 TLB 효율↑ (페이지테이블 leaf를 상위 레벨에) | 중 |
| **VM 정제: 자동 스택 성장** | 가드 페이지 + 폴트로 유저 스택 자동 확장 | 중 |
| **VM 정제: mprotect** | 런타임 페이지 권한 변경(R/W/X) — PTE 플래그 갱신 + TLB flush | 하 |
| **VM 정제: TLB/ASID** | 주소공간 전환 때 전체 flush 대신 ASID로 선택 무효화 | 중 |
| **VM 정제: ASLR** | 코드/스택/힙 배치 무작위화(보안) | 중 |
